...
function velCalc(a, b) {
  ...
  for (let i = 0; i < numrows; i++) {
    for (let j = 0; j < numcol; j++) {
      if (
        pot[i][j] == -1 &&
        abs(updatedVel[0]) + abs(updatedVel[1]) >
          abs(i - int(a / width)) + abs(j - int(b / width))
      ) {
        VEL2 = [];
...
function setup() {
  ...
  for (let i = 0; i < numObs; i++) {
    ...
    rect(
      width * int(Obs[i][0] / width),
      width * int(Obs[i][1] / width),
      width * x,
      width * y
    );
    for (let j = 0; j < x; j++) {
...
Bot[i][1]->Indicate y-coordinate of jth Bot*/
  ...
  while (i < numBots) {
    ...
    if (pot[int(x / width)][int(y / width)] == 10000) {
      ...
      fill(0, 255, 0);
      rect(
        width * int(Bot[i][0] / width),
        width * int(Bot[i][1] / width),
        width,
        width
      );
      i++;
...
function mouseDragged() {
  pot[int(mouseX / width)][int(mouseY / width)] = 10000000;
    ...
    for (let i = 0; i < numBots; i++) {
      ...
      if (abs(newVel[0]) > abs(newVel[1])) {
        ...
        for (let k = 0; k < numBots; k++) {
          if (
            i != k &&
            ((int((Bot[i][0] + 2 * velx) / width) == int(Bot[k][0] / width) &&
              int(Bot[i][1] / width) == int(Bot[k][1] / width)) ||
              (int((Bot[i][0] + 2 * velx) / width) == int(Bot[k][0] / width) &&
                int(Bot[i][1] / width) + 1 == int(Bot[k][1] / width)) ||
              (int((Bot[i][0] + 2 * velx) / width) == int(Bot[k][0] / width) &&
                int(Bot[i][1] / width) - 1 == int(Bot[k][1] / width)))
          ) {
            f = 1;
        ...
        if (
          f == 0 &&
          pot[int((Bot[i][0] + velx) / width)][int(Bot[i][1] / width)] == 10000
        ) {
          if (pot[int(Bot[i][0] / width) + 1][int(Bot[i][1] / width)] != 10000)
            Bot[i][0] = Bot[i][0] + width;
          else if (
            pot[int(Bot[i][0] / width) - 1][int(Bot[i][1] / width)] != 10000
          )
            Bot[i][0] = Bot[i][0] - width;
          else if (
            pot[int(Bot[i][0] / width)][int(Bot[i][1] / width) + 1] != 10000
          )
            Bot[i][1] = Bot[i][1] + width;
        ...
        for (let k = 0; k < numBots; k++) {
          if (
            i != k &&
            ((int((Bot[i][1] + 2 * vely) / width) == int(Bot[k][1] / width) &&
              int(Bot[i][0] / width) == int(Bot[k][0] / width)) ||
              (int((Bot[i][1] + 2 * vely) / width) == int(Bot[k][1] / width) &&
                int(Bot[i][0] / width) + 1 == int(Bot[k][0] / width)) ||
              (int((Bot[i][1] + 2 * vely) / width) == int(Bot[k][1] / width) &&
                int(Bot[i][0] / width) - 1 == int(Bot[k][0] / width)))
          ) {
            f = 1;
        ...
        if (
          f == 0 &&
          pot[int(Bot[i][0] / width)][int((Bot[i][1] + vely) / width)] == 10000
        ) {
          if (pot[int(Bot[i][0] / width)][int(Bot[i][1] / width) - 1] != 10000)
            Bot[i][1] = Bot[i][1] - width;
          else if (
            pot[int(Bot[i][0] / width)][int(Bot[i][1] / width) + 1] != 10000
          )
      ...
      //setting potential of visited cells as 0
      pot[int(Bot[i][0] / width)][int(Bot[i][1] / width)] = 0;
      if (
        x != 0 &&
        pot[int(Bot[i][0] / width) - 1][int(Bot[i][1] / width)] != 10000
      )
        pot[int(Bot[i][0] / width) - 1][int(Bot[i][1] / width)] = 0;
      if (
        y != numrows - 1 &&
        pot[int(Bot[i][0] / width)][int(Bot[i][1] / width) - 1] != 10000
      )
        pot[int(Bot[i][0] / width)][int(Bot[i][1] / width) - 1] = 0;
      if (
        y != numrows - 1 &&
        x != 0 &&
        pot[int(Bot[i][0] / width) - 1][int(Bot[i][1] / width) - 1] != 10000
      )
        pot[int(Bot[i][0] / width) - 1][int(Bot[i][1] / width) - 1] = 0;
      if (
        x != numcol - 1 &&
        pot[int(Bot[i][0] / width) + 1][int(Bot[i][1] / width)] != 10000
      )
      ...
      fill(0, 255, 0); //fill green in bot's new location
      rect(
        width * int(Bot[i][0] / width),
        width * int(Bot[i][1] / width),
        width,
      ...
      for (var p = 0; p < numgoals; p++) {
        if (
          int(Bot[i][0] / width) == int(Goal[p][0] / width) &&
          int(Bot[i][1] / width) == int(Goal[p][1] / width)
        )
          flag1 = 1;
        if (
          x != 0 &&
          int(Bot[i][0] / width) - 1 == int(Goal[p][0] / width) &&
          int(Bot[i][1] / width) == int(Goal[p][1] / width)
        )
          flag1 = 1;
        if (
          y != numrows - 1 &&
          int(Bot[i][0] / width) == int(Goal[p][0] / width) &&
          int(Bot[i][1] / width) - 1 == int(Goal[p][1] / width)
        )
          flag1 = 1;
        if (
          y != numrows - 1 &&
